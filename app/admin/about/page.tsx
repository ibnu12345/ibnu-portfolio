        ...
        <div style={{ maxWidth: '800px' }}>
          ...
          <div style={{ marginBottom: '48px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              ...
              <button onClick={() => setEditEdu({ ...emptyEdu })}
                style={{ background: '#4f46e5', color: 'white', border: 'none', borderRadius: '8px', padding: '8px 16px', fontSize: '13px', cursor: 'pointer' }}>
                + Tambah
              </button>
            </div>
                ...
                <div style={{ display: 'grid', gap: '12px' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '12px' }}>
                    <div>
                      ...
                      <input value={editEdu.year} onChange={e => setEditEdu(p => p ? { ...p, year: e.target.value } : p)} style={inputStyle} placeholder="2020–2024" />
                    </div>
                    <div>
                      <label style={labelStyle}>Gelar / Program Studi</label>
                      <input value={editEdu.degree} onChange={e => setEditEdu(p => p ? { ...p, degree: e.target.value } : p)} style={inputStyle} placeholder="S1 Pendidikan Bahasa Arab" />
                    </div>
                  </div>
                  <div>
                    <label style={labelStyle}>Institusi</label>
                    <input value={editEdu.institution} onChange={e => setEditEdu(p => p ? { ...p, institution: e.target.value } : p)} style={inputStyle} placeholder="UIN Siber Syekh Nurjati Cirebon" />
                  </div>
                  <div>
                    <label style={labelStyle}>Deskripsi</label>
                    <textarea value={editEdu.description} onChange={e => setEditEdu(p => p ? { ...p, description: e.target.value } : p)} rows={3} style={{ ...inputStyle, resize: 'vertical' }} placeholder="Deskripsi singkat..." />
                  </div>
                  <div>
                    <label style={labelStyle}>Urutan (sort order)</label>
                    <input type="number" value={editEdu.sort_order} onChange={e => setEditEdu(p => p ? { ...p, sort_order: parseInt(e.target.value) } : p)} style={{ ...inputStyle, width: '100px' }} />
                  </div>
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              ...
              <button onClick={() => setEditOrg({ ...emptyOrg })}
                style={{ background: '#4f46e5', color: 'white', border: 'none', borderRadius: '8px', padding: '8px 16px', fontSize: '13px', cursor: 'pointer' }}>
                + Tambah
              </button>
            </div>
              ...
              <div style={{ background: 'rgba(168,85,247,0.05)', border: '1px solid rgba(168,85,247,0.2)', borderRadius: '16px', padding: '24px', marginBottom: '16px' }}>
                <div style={{ display: 'grid', gap: '12px' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '12px' }}>
                    <div>
                      <label style={labelStyle}>Tahun</label>
                      <input value={editOrg.year} onChange={e => setEditOrg(p => p ? { ...p, year: e.target.value } : p)} style={inputStyle} placeholder="2021–2022" />
                    </div>
                    <div>
                      <label style={labelStyle}>Posisi / Jabatan</label>
                      <input value={editOrg.position} onChange={e => setEditOrg(p => p ? { ...p, position: e.target.value } : p)} style={inputStyle} placeholder="Ketua Divisi Penelitian" />
                    </div>
                  </div>
                  <div>
                    <label style={labelStyle}>Nama Organisasi</label>
                    <input value={editOrg.org_name} onChange={e => setEditOrg(p => p ? { ...p, org_name: e.target.value } : p)} style={inputStyle} placeholder="HMJ Pendidikan Bahasa Arab" />
                  </div>
                  <div>
                    <label style={labelStyle}>Deskripsi</label>
                    <textarea value={editOrg.description} onChange={e => setEditOrg(p => p ? { ...p, description: e.target.value } : p)} rows={3} style={{ ...inputStyle, resize: 'vertical' }} />
                  </div>
                  <div>
                    <label style={labelStyle}>Urutan (sort order)</label>
                    <input type="number" value={editOrg.sort_order} onChange={e => setEditOrg(p => p ? { ...p, sort_order: parseInt(e.target.value) } : p)} style={{ ...inputStyle, width: '100px' }} />
                  </div>
                  <div style={{ display: 'flex', gap: '8px' }}>
