...
export default function AdminResearch() {
  ...
  async function handleSave() {
    setSaving(true)
    if (form.id) await supabase.from('research').update({ ...form, updated_at: new Date().toISOString() }).eq('id', form.id)
    else await supabase.from('research').insert(form)
    setSaving(false); setShowForm(false); setForm(empty); load()
  }

  async function handleDelete(id: string) {
    if (!confirm('Hapus penelitian ini?')) return
    await supabase.from('research').delete().eq('id', id); load()
  }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        ...
        <button onClick={() => { setForm(empty); setShowForm(true) }} style={{ background: '#4f46e5', color: 'white', border: 'none', borderRadius: '8px', padding: '10px 20px', fontSize: '14px', cursor: 'pointer' }}>+ Tambah Research</button>
      </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '20px' }}>
          <div style={{ background: '#111118', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '16px', padding: '32px', width: '100%', maxWidth: '560px', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'grid', gap: '14px' }}>
              {[{ key: 'title', label: 'Judul *', type: 'text' }, { key: 'journal_name', label: 'Nama Jurnal', type: 'text' }, { key: 'published_date', label: 'Tanggal Publikasi', type: 'date' }, { key: 'video_url', label: 'Video URL', type: 'text' }, { key: 'pdf_url', label: 'PDF URL', type: 'text' }].map(f => (
                <div key={f.key}>
      ...
      <div style={{ display: 'grid', gap: '12px' }}>
        {items.length === 0 && <div style={{ textAlign: 'center', padding: '60px', color: 'rgba(255,255,255,0.3)' }}>Belum ada research.</div>}
        {items.map(item => (
